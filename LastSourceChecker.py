...
for r in raw_data:
    if exif == r[3] and hash == r[4]:
        Last_Source_Candidate.append(r[1])
...
if len(Last_Source_Candidate) == 0:
    ...
    for c in Last_Source_Candidate:
        for f in filename_data:
            if (c == f[1]) and (f[3] in input_image_filename):
                print("Last source of '{}' : ".format(image), end='')
                print('\033[31m' + c + '\033[0m')
                check += 1
    
    # Editor_Signature 자체에 특정 editor가 없을 경우, 다른 LSC의 특징에 맞지 않는 LSC는 remove
    if check == 0:
        count = len(Last_Source_Candidate)
        idx = 0
        for i in range(count):
            for f in filename_data:
                if (Last_Source_Candidate[idx] == f[1]) and (f[3] not in input_image_filename):
                    Last_Source_Candidate.remove(f[1])
